...
class CompaniesServices{
  ...
  Future<Getallcompaniesmodel?> Gerallcompanies() async {
    ...
    final url = baseurl+ 'company'; // Replace with your actual endpoint
  ...
  Future<LocationModel?> Getallcompaniesbyid(String id) async {
    ...
    final url = baseurl+'company/get_company_locations'; // Replace with your actual endpoint
  ...
  //Get company by id
  Future<GetAllAreas?> Getallareasbyid(String id) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('token');
    String? uid = prefs.getString('uid');
    ...
    final url = baseurl+'company/get_location_areas'; // Replace with your actual endpoint
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'location_id': id, // Add additional parameters if required
        }),
      ...
      if (response.statusCode == 200) {
        // Parse the JSON response and convert it to a User object
        Map<String, dynamic> responseData = json.decode(response.body);
        return GetAllAreas.fromJson(responseData);
      } else {
        print('Failed to fetch user data: ${response.body}');
        return null;
      }
